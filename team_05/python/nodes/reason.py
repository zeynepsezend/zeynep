...
- **Slab cost when a thickness AND material are given (e.g. "slab thickness 0.4 m, post_tensioned")** → call `compute_slab_cost` with `room_name`, `thickness_m`, `material`. The tool resolves area from the layout, looks up the per-m3 rate from `cost_rates.json` (slab_material) and returns `area * thickness * rate`. If the user lists multiple slabs for the same room, call `compute_slab_cost` once per slab and sum the results yourself in the final answer. Do NOT respond with `DATA_MISSING` for slab unit prices — this tool supplies them.
